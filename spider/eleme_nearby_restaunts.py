...
import json
import copy
from .eleme_restaunt import ElemeRestaunt
...
class ElemeNearbyRestauntManager():
    ...
    IMAGE_URL = 'http://fuss10.elemecdn.com'

    def __init__(self):
        '''
        @note No results store in restaunt_detail, it only describe the data structure
        ...
        self.restaunt_detail = {'minimum_free_delivery_amount': 0,
                                ...
                                'delivery_fee': 0, 'id': 0, 'image_path': ''}
        # 'image_path' need to contact with the relative url
    ...
    def _parse_fast_food(self, json_fastfoods):
        ...
        for restaunt in restaunts:
            ...
            detail['image_path'] = self.IMAGE_URL + restaunt['image_path']
            restaunts_nearby.append(detail)
        # print(restaunts_nearby)
    ...
    def restuants(self, location):
        ...
        request 3:Get classfication of kinds shops
        request 4:
        No use 5 and 6
        '''
        ...
        request = urllib.request.Request(method="POST", url=self.URL_ELEME_API, data=url_values, headers=header_dict)
